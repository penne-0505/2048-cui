...
def get_default_save_path() -> str:
    ...
    if system == "Windows":
        ...
        return str(Path.home() / "Library" / "Application Support" / "2048-cli" / "saves")
...
def get_save_dir(config: Optional[Dict[str, Any]] = None) -> str:
...
def save_game(game: Any, slot: int, name: Optional[str] = None, config: Optional[Dict[str, Any]] = None) -> bool:
    """Save game to specified slot. Returns True if successful, False otherwise."""
    try:
        save_dir = get_save_dir(config)
        if not ensure_save_dir_exists(save_dir):
            return False
        file_path = os.path.join(save_dir, f"{SAVE_SLOT_PREFIX}{slot}{SAVE_FILE_EXTENSION}")
...
def load_game(game: Any, slot: int, config: Optional[Dict[str, Any]] = None) -> bool:
...
def get_save_slots(config: Optional[Dict[str, Any]] = None) -> List[str]:
    ...
    try:
        ...
        return [f for f in files if f.startswith(SAVE_SLOT_PREFIX) and f.endswith(SAVE_FILE_EXTENSION)]
    except (OSError, PermissionError) as e:
...
def get_save_slot_info(slot: int, config: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    ...
    except (json.JSONDecodeError, KeyError, IOError, OSError, PermissionError, ValueError) as e:
...
def get_all_save_slots_info(config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
