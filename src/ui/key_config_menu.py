...
from core.config import save_config, get_save_path, set_save_path
        ...
        if choice == "Movement Keys":
            ...
            if confirm_reset_keys(stdscr):
                if reset_to_defaults(config):
                    ...
                    show_message(stdscr, "Key bindings reset but failed to save configuration")
...
def configure_movement_keys(stdscr: curses.window, config: Dict[str, Any]) -> None:
    ...
    while True:
        ...
        for action in ["up", "down", "left", "right"]:
            ...
            options.append(f"{get_action_display_name('movement', action)}: {key_display}")
...
def configure_action_keys(stdscr: curses.window, config: Dict[str, Any]) -> None:
    ...
    while True:
        ...
        for action in ["quit", "save", "return_to_title", "load", "change_theme"]:
            ...
            key_display = ", ".join(get_key_display_name(key) for key in keys)
            options.append(f"{get_action_display_name('actions', action)}: {key_display}")
...
def configure_single_action(stdscr: curses.window, config: Dict[str, Any], category: str, action: str) -> None:
                ...
                if get_key_display_name(key) == key_to_remove:
                    ...
                    if success:
                        if save_config(config):
                            show_message(stdscr, f"Removed key binding: {get_key_display_name(key)}")
                        else:
                            show_message(stdscr, "Key binding removed but failed to save configuration")
                    else:
                        show_message(stdscr, f"Error: {error}")
                    break


def add_new_key_binding(stdscr: curses.window, config: Dict[str, Any], category: str, action: str) -> None:
    ...
    if not is_key_safe(key_str):
        show_message(stdscr, f"Key '{get_key_display_name(key_str)}' is not in the safe bindable keys list")
        return
...
def show_key_bindings_help(stdscr: curses.window, config: Dict[str, Any]) -> None:
    ...
    for action in ["up", "down", "left", "right"]:
        keys = config["keys"]["movement"][action]
        key_display = ", ".join(get_key_display_name(key) for key in keys)
        stdscr.addstr(y, 3, f"{get_action_display_name('movement', action)}: {key_display}")
        y += 1
    ...
    for action in ["quit", "save", "return_to_title", "load", "change_theme"]:
        keys = config["keys"]["actions"][action]
        key_display = ", ".join(get_key_display_name(key) for key in keys)
        stdscr.addstr(y, 3, f"{get_action_display_name('actions', action)}: {key_display}")
        y += 1
        ...
        if choice == "Set Custom Path":
                ...
                if new_path.strip() == "":
                    ...
                    try:
                        ...
                        if set_save_path(config, new_path):
                            ...
                            show_message(stdscr, "Path set but failed to save configuration")
            ...
            if confirm_reset_save_path(stdscr):
                if set_save_path(config, None):
                    ...
                    show_message(stdscr, "Save path reset but failed to save configuration")
